...
class PeerConnection {
  ...
  /* receive state */
  _receiveBuffer: ArrayBuffer[];
  _receivedSize: number;
  _fileMeta: any;
  _isNative: boolean;
  _nativePath: string | null;
  _receiveReady: Promise<any>;
  _writeChain: Promise<any>;
  _writeError: any;
  ...
  constructor(jobId = 'unknown') {
    ...
    this._fileMeta = null;
    this._isNative = !!(window as any).api;
  ...
  async handleIceCandidate(msg: any) {
  ...
  async sendFile(file: File | string, meta: any) {
    if (this._sending) return Promise.reject(new Error('Already sending a file'));
    this._sending = true;

    if (typeof file === 'string' && !!(window as any).api) {
    ...
    return new Promise<void>(async (resolve, reject) => {
      ...
      try {
        const metaMsg = JSON.stringify({ type: 'file-meta', fileName: fileObj.name, fileSize: fileObj.size, ...meta });
  ...
  async _sendNativeFile(filePath: string, meta: any) {
    return new Promise<void>(async (resolve, reject) => {
      ...
      try {
        const fileSize = await (window as any).api.getFileSize(filePath);
        ...
        while (offset < fileSize) {
          ...
          const chunk = await (window as any).api.readOutputFileChunk(filePath, offset, end);
  ...
  _setupDataChannelEvents(channel: RTCDataChannel) {
      ...
      if (typeof event.data === 'string') {
        try {
          ...
          if (msg.type === 'file-meta') {
            ...
            if (this._isNative) {
              ...
              this._receiveReady = (window as any).api.createTempWriteStream(this.jobId, ext).then((path: string) => {
            ...
            if (this._isNative) {
              ...
              await (window as any).api.finishTempWrite(this.jobId);
        ...
        if (this._isNative) {
          ...
          this._writeChain = this._writeChain.then(async () => {
            ...
            try {
              await this._receiveReady;
              await (window as any).api.writeTempChunk(this.jobId, chunk);
            } catch (err) {
